// Package codegen wraps coding-agent CLIs (Claude Code, Codex, Aider, etc.)
// behind a small Agent interface so the rest of the system can stay agnostic
// to which tool actually edits the user's repository.
//
// An Agent is given a prompt and a working directory; it shells out to a CLI
// that is expected to read the prompt from stdin and make whatever changes it
// wants inside workDir. The combined stdout+stderr is captured (with an
// optional byte cap) and returned in a Result.
package codegen
